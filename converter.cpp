...
void Converter::run() {
	...
	foreach (QString file2do, Files) {
		emit progress(now / max, "parse " + file2do);
		now += 1;
		if (parseProFile(file2do)) {
			...
			emit progress(now / max, "nix damit: " + file2do);
			now += 2;
		}
	}
}
...
void Converter::setBaseDir(QString basis) {
	...
	QRegExp templ("\\s*TEMPLATE\\s*[+]?=(.*)"),
		subre("\\s*SUBDIRS\\s*[+]?=(.*)", Qt::CaseSensitive, QRegExp::RegExp2);
	int i;
				...
				if (string.contains("subdirs", Qt::CaseInsensitive)) {
					...
					if (i > -1) {
						...
						if (string.contains('"')) {
							...
							liste = string.split(QRegExp("\\s+"),
												 QString::SkipEmptyParts);
						}
						foreach (string, liste)
							dirs.append(
								QString("%1/%2").arg(dirs.first()).arg(string));
					}
				}
			}
		}
...
bool Converter::parseProFile(QString proFile) {
	...
	QStringList lines;
	QString work;
	QRegExp liner("^\\s*(\\w+)\\s*([-+]?=)\\s*(.*)$", Qt::CaseInsensitive,
				  QRegExp::RegExp2);
	int op;
	QStringList *sl;
	...
	if (file.open(QIODevice::ReadOnly)) {
		...
		qDebug() << "TEMPLATE:" << TEMPLATE << ", TARGET:" << TARGET
				 << ", DESTDIR:" << DESTDIR;
	} else
		return false;
	return true;
...
void Converter::operateOnVar(QStringList *&var, int &op, QString val) {
	...
	QStringList lst =
		v.split(QRegExp("\"[+]{3}\\s*\"?"), QString::SkipEmptyParts);
	v = lst.takeFirst();
	v.remove('"');
	lst.insert(0, v);
	v = lst.takeLast();
	...
	switch (op) {
	case -1: //"-="
		foreach (QString item, lst) { var->removeAll(item); }
		break;
	case 0:
		var->clear();
	case 1:
...
bool Converter::findQOMacro(QString fn) {
	QRegExp qom("^.*Q_OBJECT\\s*$");
	QFile file(fn);
	bool found(false);

	if (file.open(QIODevice::ReadOnly)) {
		while (!found && !file.atEnd()) {
			if (qom.exactMatch(file.readLine()))
				found = true;
		}
		file.close();
		return found;
...
void Converter::writeCMakeLists(QString proFile) {
	...
	if (file.open(QIODevice::WriteOnly)) {
		...
		QRegExp buildtype(".*(debug|release|debug_and_release).*",
						  Qt::CaseInsensitive);
		...
		if (!DESTDIR.isEmpty()) {
			...
			if (TEMPLATE.contains("lib"))
				text
					<< QString("set ( LIBRARY_OUTPUT_PATH %1 )\n").arg(DESTDIR);
		}
		...
		if (TEMPLATE.contains("subdirs", Qt::CaseInsensitive))
			...
			if (!TOMOC.isEmpty()) {
				text << QString("set ( %1_MOCS\n\t%2\n\t)\n")
							...
							.arg(TOMOC.join("\n\t"));
				text
					<< QString("QT5_WRAP_CPP(MOCS ${%1_MOCS})\n\n").arg(TARGET);
				help += " ${MOCS}";
			}
			if (TEMPLATE == "app") {
			...
			text
				<< QString(
					   "target_link_libraries ( %1 %2 Qt5::Core Qt5::Gui Qt5::Widgets )\n")
					   .arg(TARGET)
					   .arg(links);
		}
		text.flush();
