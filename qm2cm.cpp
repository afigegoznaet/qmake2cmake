...
qm2cm::qm2cm() : QMainWindow(), conv(), progress(this) {
	setupUi(this);
...
void qm2cm::startFolderSelect() {
	QString f = QFileDialog::getExistingDirectory(
		this, tr("Please select the base Folder of your QMake project"));
	if (!f.isEmpty()) {
		le_base->setText(f);
		folderSelected();
	}
...
void qm2cm::startConversion() {
	...
	connect(&conv, SIGNAL(progress(qreal, QString)), this,
			SLOT(showProgress(qreal, QString)));
	connect(&conv, SIGNAL(finished()), this, SLOT(convDone()));
	conv.start();
}
